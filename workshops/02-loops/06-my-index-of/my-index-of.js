...
const myIndexOf = (source, searchedValue, startIdx) => {
    if (startIdx === undefined) {
        startIdx = 0;
    }

    for (let i = startIdx; i <= source.length - searchedValue.length; i++) {
        let substring = source.slice(i, i + searchedValue.length);

        if (substring === searchedValue) {
            return i;
        }
    }
